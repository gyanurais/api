...
def delete_post(id: int, db:session=Depends(get_db), current_user:int=Depends(oauth2.get_current_user)):
    ...
    if postt.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not authosized to perform requestedaction")
...
# update
@router.put("/{id}", response_model=schema.Postmeth)
def update_post(id: int, posts: schema.Postcreate, db:session=Depends(get_db), current_user:int=Depends(oauth2.get_current_user)):
    ...
    if postt.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to perform requestedaction")

    post.update(posts.dict(), synchronize_session=False)
    db.commit()
    return post.first()
